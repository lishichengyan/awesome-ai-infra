...
# 1,2,3 -> 0,1,2
# aligns with with PyTorch’s expectation that class indices begin at 0
# for models and loss functions like CrossEntropyLoss.
labels = torch.tensor(labels, dtype=torch.long) - 1

# print("vectors and labels: ", vectors, labels)
...
new_docs = [
    ...
    "Let's watch a fun movie.",
]
class_names = ["Cinema", "Music", "Science"]
new_doc_vectors = torch.tensor(
    [doc_to_bow(new_doc, vocabulary) for new_doc in new_docs], dtype=torch.float32
)
with torch.no_grad():
